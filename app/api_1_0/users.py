...
def get_user_posts(id):
    ...
    pagination = user.posts.order_by(Post.timstamp.desc()).paginate(
...
def get_user_followed_posts(id):
    ...
    pagination = user.followed_posts.order_by(Post.timstamp.desc()).paginate(
        page, per_page=current_app.config['FLASK_POSTS_PER_PAGE'],
        error_out=False)
    posts = pagination.items
    prev = None
    if pagination.has_prev:
        prev = url_for('aip.get_user_followed_posts', page=page-1, 
                    _external=True)
    next = None
    if pagination.has_next:
